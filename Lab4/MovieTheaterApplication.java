...
public class MovieTheaterApplication {
    ...
    public Integer getShowingsCount(String thePriceCode) throws SQLException
    {
        ...
        String st = "SELECT COUNT(*) FROM Showings WHERE priceCode=?";
        try{
            PreparedStatement showStmt = connection.prepareStatement(st);
            showStmt.setString(1, thePriceCode);
            showStmt.executeQuery();
            ResultSet S_result = showStmt.getResultSet();
            S_result.next();
            result = S_result.getInt(1);
    ...
    public int updateMovieName(int theMovieID, String newMovieName)  throws SQLException
    {
        ...
        Integer result2 = 0;
        String st2 = "UPDATE Movies SET name=? WHERE movieID=?";
        try{
            PreparedStatement movieStmt = connection.prepareStatement(st2);
            movieStmt.setString(1, newMovieName);
            movieStmt.setInt(2, theMovieID);
            result2 = movieStmt.executeUpdate();
        }catch(SQLException e){
                e.printStackTrace();
                System.exit(-1);
        }
    ...
    public int reduceSomeTicketPrices (int maxTicketCount)
    {
        ...
        try{ 
            ...
            PreparedStatement laststmt = connection.prepareStatement(query);
            laststmt.setInt(1, maxTicketCount);
            laststmt.executeQuery();
            ResultSet lastRS = laststmt.getResultSet();
            if (lastRS.next()){
                storedFunctionResult = lastRS.getInt(1);
            }
            laststmt.close();
        }catch(SQLException e){
                e.printStackTrace();
